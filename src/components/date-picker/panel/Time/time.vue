        ...
        <div :class="[prefixCls + '-body']">
            ...
            <div :class="[prefixCls + '-content']">
                <time-spinner
                    ...
                    :disabled-minutes="
                        disabledHMS.disabledMinutes
                    "
                    :disabled-seconds="
                        disabledHMS.disabledSeconds
                    "
...
export default {
    ...
    computed: {
        ...
        timeSlots() {
            ...
            return ['getHours', 'getMinutes', 'getSeconds'].map(
                slot => this.date[slot]()
            );
        },
        disabledHMS() {
            ...
            if (
                this.disabledDate === returnFalse ||
                !this.value[0]
            ) {
                const disabled = disabledTypes.reduce(
                    (obj, type) => (
                        (obj[type] = this[type]), obj
                    ),
                    {}
                );
                return disabled;
                ...
                const disabledHMS = disabled.map(
                    (preDisabled, j) => {
                        const slot = slots[j];
                        ...
                        for (
                            ...
                            const hms = this.timeSlots.map(
                                (slot, x) => (x === j ? i : slot)
                            );
                            const testDateTime = mergeDateHMS(
                                this.date,
                                ...hms
                            );
                            if (
    ...
    mounted() {
        if (
            this.$parent &&
            this.$parent.$options.name === 'DatePicker'
        )
            this.showDate = true;
    }
};
