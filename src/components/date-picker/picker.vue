...
export default {
    ...
    data() {
        ...
        return {
            ...
            isFocused: false,
            focusedDate:
                initialValue[0] || this.startDate || new Date(),
            focusedTime: {
                ...
                if (
                    ...
                    if (this.$IVIEW) {
                        if (this.$IVIEW.timePicker.iconSize) {
                            size =
                                this.$IVIEW.timePicker.iconSize;
                        }
                    }
                } else {
                    if (this.$IVIEW) {
                        if (this.$IVIEW.datePicker.iconSize) {
                            size =
                                this.$IVIEW.datePicker.iconSize;
                        }
                    }
                }
            }
    ...
    methods: {
        onSelectionModeChange(type) {
            ...
            this.selectionMode =
                oneOf(type, ['year', 'month', 'date', 'time']) &&
                type;
        ...
        handleClose(e) {
            ...
            if (this.visible) {
                const pickerPanel =
                    this.$refs.pickerPanel &&
                    this.$refs.pickerPanel.$el;
                if (
                    e &&
                    pickerPanel &&
                    pickerPanel.contains(e.target)
                )
                    return; // its a click inside own component, lets ignore it.
        ...
        handleKeydown(e) {
            ...
            if (keyCode === 9) {
                if (this.visible) {
                    ...
                    if (this.isConfirm) {
                        ...
                        const tabbable =
                            this.$refs.drop.$el.querySelectorAll(
                                selector
                            );
            ...
            if (keyCode === 13) {
                const timePickers = findComponentsDownward(
                    this,
                    'TimeSpinner'
                );
                if (timePickers.length > 0) {
                    const columnsPerPicker = timePickers[0]
                        .showSeconds
                        ? 3
                        : 2;
                    ...
                    const value =
                        this.focusedTime.time[pickerIndex];
                ...
                if (this.type.match(/range/)) {
                    ...
                    const panels = findComponentsDownward(
                        this,
                        'PanelTable'
                    );
                    const compareDate = d => {
                        const sliceIndex =
                            ['year', 'month', 'date'].indexOf(
                                this.type
                            ) + 1;
                        return [
                            d.getFullYear(),
            ...
            this.navigateDatePanel(
                keyValueMapper[keyCode],
                e.shiftKey
            );
        },
        reset() {
        ...
        navigateTimePanel(direction) {
            ...
            const column = (currentColumn => {
                const incremented =
                    ...
                    (horizontal
                        ...
                        : 0);
                return (
                    (incremented + maxNrOfColumns) %
                    maxNrOfColumns
                );
            })(this.focusedTime.column);

            const columnsPerPicker =
                maxNrOfColumns / timePickers.length;
            const pickerIndex = Math.floor(
                column / columnsPerPicker
            );
            ...
            if (vertical) {
                ...
                const nextIndex =
                    (currentIndex +
                        increment +
                        pickerPossibleValues.length) %
                    pickerPossibleValues.length;
                const nextValue =
                    pickerPossibleValues[nextIndex].text;
                const times = this.focusedTime.time.map(
                    (time, i) => {
        ...
        navigateDatePanel(direction, shift) {
            ...
            if (shift) {
                ...
                const position = direction.match(/left|down/)
                    ? 'prev'
                    : 'next';
                const double = direction.match(/up|down/)
                    ? '-double'
                    : '';
            ...
            if (this.type.match(/^date/)) {
                ...
                const nextDay =
                    focusedDate.getDate() +
                    mapPossibleValues(direction, 1, 7);
        ...
        handleInputChange(event) {
            const isArrayValue =
                this.type.includes('range') || this.multiple;
            const oldValue = this.visualValue;
            const newValue = event.target.value;
            const newDate = this.parseDate(newValue);
            const disabledDateFn =
                this.options &&
                typeof this.options.disabledDate ===
                    'function' &&
                this.options.disabledDate;
            const valueToTest = isArrayValue
                ? newDate
                : newDate[0];
            const isDisabled =
                disabledDateFn && disabledDateFn(valueToTest);
        ...
        parseDate(val) {
            ...
            const multipleParser =
                TYPE_VALUE_RESOLVER_MAP['multiple'].parser;
        ...
        formatDate(value) {
            ...
            if (this.multiple) {
                const formatter =
                    TYPE_VALUE_RESOLVER_MAP.multiple.formatter;
                return formatter(
                    value,
                    this.format || format,
                    this.separator
        ...
        onPick(dates, visible = false, type) {
            if (this.multiple) {
                ...
                const indexOfPickedDate =
                    this.internalValue.findIndex(
                        date =>
                            date &&
                            date.getTime() === pickedTimeStamp
                    );
                const allDates = [
                    ...this.internalValue,
                    dates
                ...
                this.internalValue = timeStamps.map(
                    ts => new Date(ts)
                );
            } else {
                dates = this.parseDate(dates);
                this.internalValue = Array.isArray(dates)
                    ? dates
                    : [dates];
            }
