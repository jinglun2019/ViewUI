    ...
    <div :class="wrapClasses" style="touch-action: none">
        ...
        >
            <div
                :class="loaderClasses"
                :style="{
                    paddingTop: wrapperPadding.paddingTop
                }"
                ref="toploader"
            >
                <loader
            ...
            <div
                :class="loaderClasses"
                :style="{
                    paddingBottom: wrapperPadding.paddingBottom
                }"
                ref="bottomLoader"
            >
                <loader
...
export default {
    ...
    data() {
        const distanceToEdge =
            this.calculateProximityThreshold();
        return {
            showTopLoader: false,
            ...
            return [
                `${prefixCls}-container`,
                {
                    [`${prefixCls}-container-loading`]:
                        this.showBodyLoader && this.stopSlide
                }
            ];
        },
    ...
    methods: {
        ...
        onCallback(dir) {
                ...
                for (let i = 0; i < 20; i++) {
                    setTimeout(() => {
                        bottomLoaderHeight = Math.max(
                            bottomLoaderHeight,
                            this.$refs.bottomLoader.getBoundingClientRect()
                                .height
                        );
                        container.scrollTop =
                            initialScrollTop +
            ...
            const callbacks = [
                this.waitOneSecond(),
                this.onReachEdge ? this.onReachEdge(dir) : noop()
            ];
            callbacks.push(
                dir > 0
                    ? this.onReachTop
        ...
        stretchEdge(direction) {
            ...
            if (direction > 0 && this.reachedTopScrollLimit) {
                this.topRubberPadding +=
                    5 - this.topRubberPadding / 5;
                if (
                    this.topRubberPadding >
                    this.topProximityThreshold
                )
                    this.onCallback(1);
            } else if (
                direction < 0 &&
                ...
                this.bottomRubberPadding +=
                    6 - this.bottomRubberPadding / 4;
                if (
                    this.bottomRubberPadding >
                    this.bottomProximityThreshold
                )
                    this.onCallback(-1);
            } else {
                this.onScroll();
        ...
        onScroll() {
            ...
            ); // IE has no Math.sign, check that webpack polyfills this
            const displacement =
                el.scrollHeight - el.clientHeight - el.scrollTop;

            const topNegativeProximity =
                this.topProximityThreshold < 0
                    ? this.topProximityThreshold
                    : 0;
            const bottomNegativeProximity =
                this.bottomProximityThreshold < 0
                    ? this.bottomProximityThreshold
                    : 0;
            if (
                scrollDirection == -1 &&
                displacement + bottomNegativeProximity <=
                    dragConfig.sensitivity
            ) {
                this.reachedBottomScrollLimit = true;
            } else if (
                scrollDirection >= 0 &&
                el.scrollTop + topNegativeProximity <= 0
            ) {
        ...
        onPointerMove(e) {
            ...
            const yDiff =
                pointerPosition.y - this.pointerTouchDown.y;
            ...
            if (!this.touchScroll) {
                const wasDragged =
                    Math.abs(yDiff) >
                    dragConfig.minimumStartDragOffset;
