...
export default {
        ...
        isSelectAll() {
            ...
            for (let i in this.objData) {
                ...
                if (
                    ...
                    isSelectAll = this.isChildrenSelected(
                        objData,
                        isSelectAll
                    );
                }
                if (
                    !(objData._isDisabled && !objData._isChecked)
                ) {
                    ...
                    isAllDisabledAndUnSelected =
                        this.isChildrenAllDisabledAndUnSelected(
                            objData,
                            isAllDisabledAndUnSelected
                        );
                }
    ...
    methods: {
        ...
        handleMouseDown(column, event) {
            ...
            if (this.draggingColumn) {
                ...
                const handleMouseMove = event => {
                    ...
                    const proxyLeft =
                        this.dragState.startLeft + deltaLeft;
                ...
                const handleMouseUp = () => {
                    if (this.dragging) {
                        const { startColumnLeft, startLeft } =
                            this.dragState;

                        const finalLeft = parseInt(
                            resizeProxy.style.left,
                            10
                        );
                        const columnWidth =
                            finalLeft - startColumnLeft;
            ...
            if (objData.children && objData.children.length) {
                objData.children.forEach(row => {
                    if (!row._isChecked && !row._isDisabled) {
                        ...
                        status = this.isChildrenSelected(
                            row,
                            status
                        );
                    }
        ...
        isChildrenAllDisabledAndUnSelected(
            objData,
            isAllDisabledAndUnSelected
        ) {
            ...
            if (objData.children && objData.children.length) {
                objData.children.forEach(row => {
                    if (!row._isDisabled) {
                        ...
                        status = this.isChildrenDisabled(
                            row,
                            status
                        );
                    }
