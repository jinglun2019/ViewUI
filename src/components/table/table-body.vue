...
export default {
    ...
    methods: {
        rowChecked(_index) {
            return (
                this.objData[_index] &&
                this.objData[_index]._isChecked
            );
        },
        rowDisabled(_index) {
            return (
                this.objData[_index] &&
                this.objData[_index]._isDisabled
            );
        },
        rowExpanded(_index) {
            return (
                this.objData[_index] &&
                this.objData[_index]._isExpanded
            );
        },
            ...
            if (data.children && data.children.length) {
                for (let i = 0; i < data.children.length; i++) {
                    ...
                    if (row[this.rowKey] === rowKey) {
                        ...
                        const child = this.getChildLevel(
                            row,
                            rowKey,
                            level + 1
                        );
                    ...
                    const $tableTr = h(
                        TableTr,
                        {
                            ...
                            nativeOn: {
                                ...
                                click: e =>
                                    this.clickCurrentRow(
                                        row._index,
                                        e,
                                        row._rowKey
                                    ),
                                dblclick: e =>
                                    this.dblclickCurrentRow(
                                        row._index,
                                        e,
                                        row._rowKey
                                    ),
        ...
        this.data.forEach((row, index) => {
            ...
            const $tableTr = h(
                TableTr,
                {
                    ...
                    nativeOn: {
                        mouseenter: e =>
                            this.handleMouseIn(row._index, e),
                        mouseleave: e =>
                            this.handleMouseOut(row._index, e),
                        click: e =>
                            this.clickCurrentRow(row._index, e),
                        dblclick: e =>
                            this.dblclickCurrentRow(
                                row._index,
                                e
            ...
            // 子数据
            if (row.children && row.children.length) {
                const $childNodes = this.getChildNode(
                    h,
                    row,
                    []
                );
