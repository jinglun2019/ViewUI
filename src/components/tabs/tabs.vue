...
export default {
    ...
    methods: {
        getTabs() {
            ...
            const AllTabPanes = findComponentsDownward(
                this,
                'TabPane'
            );
            const TabPanes = [];
        ...
        updateBar() {
            this.$nextTick(() => {
                ...
                this.barWidth = tab
                    ? parseFloat(tab.offsetWidth)
                    : 0;

                if (index > 0) {
                    let offset = 0;
                    const gutter =
                        this.size === 'small' ? 0 : 16;
                    for (let i = 0; i < index; i++) {
                        offset +=
                            parseFloat(prevTabs[i].offsetWidth) +
                            gutter;
                    }
            ...
            return [
                `${prefixCls}-tab`,
                {
                    ...
                    [`${prefixCls}-tab-active`]:
                        item.name === this.activeKey,
                    [`${prefixCls}-tab-focused`]:
                        item.name === this.focusedKey
                }
            ];
        },
        handleChange(index) {
        ...
        handleContextmenu(index, event) {
            ...
            this.$nextTick(() => {
                ...
                const TabsBounding =
                    $TabsWrap.getBoundingClientRect();
                const position = {
                    left: `${
                        event.clientX - TabsBounding.left
                    }px`,
            ...
            if (tab.currentName === this.activeKey) {
                ...
                if (newTabs.length) {
                    ...
                    if (rightNoDisabledTabs.length) {
                        activeKey =
                            rightNoDisabledTabs[0].currentName;
                    } else if (leftNoDisabledTabs.length) {
                        activeKey =
                            leftNoDisabledTabs[
                                leftNoDisabledTabs.length - 1
                            ].currentName;
                    } else {
        ...
        scrollToActiveTab() {
            ...
            if (
                activeTabBounding.left < navScrollBounding.left
            ) {
                newOffset =
                    currentOffset -
                    (navScrollBounding.left -
                        activeTabBounding.left);
            } else if (
                activeTabBounding.right > navScrollBounding.right
            ) {
                newOffset =
                    currentOffset +
        ...
        updateVisibility(index) {
            [
                ...this.$refs.panes.querySelectorAll(
                    `.${prefixCls}-tabpane`
                )
            ].forEach((el, i) => {
                if (index === i) {
        ...
        handleDrop(index, event) {
            ...
            if (nav) {
                const dragName =
                    event.dataTransfer.getData('tab-name');
                event.preventDefault();

                let navNames = this.navList.map(
                    item => item.name
                );
                const a = parseInt(
                    navNames.findIndex(item => item === dragName)
                );
                const b = parseInt(
                    navNames.findIndex(item => item === nav.name)
                );
                navNames.splice(
                    b,
                    1,
                    ...navNames.splice(a, 1, navNames[b])
                );
                this.$emit(
                    'on-drag-drop',
                    dragName,
    ...
    watch: {
        ...
        activeKey(val) {
            ...
            const nextIndex = Math.max(
                this.getTabIndex(this.focusedKey),
                0
            );
