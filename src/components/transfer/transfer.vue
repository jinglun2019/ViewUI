            ...
            [
                ...
                h(
                    List,
                    {
                        ...
                        on: {
                            'on-checked-keys-change':
                                this.handleRightCheckedKeysChange
                        }
                    },
                    clonedVNodes
                )
        ...
        getValidKeys(direction) {
            return this[`${direction}Data`]
                .filter(
                    data =>
                        !data.disabled &&
                        this[`${direction}CheckedKeys`].indexOf(
                            data.key
                        ) > -1
                )
                .map(data => data.key);
        ...
        splitData(init = false) {
            ...
            if (this.targetKeys.length > 0) {
                this.targetKeys.forEach(targetKey => {
                    const filteredData = this.leftData.filter(
                        (data, index) => {
        ...
        splitSelectedKey() {
            ...
            if (selectedKeys.length > 0) {
                this.leftCheckedKeys = this.leftData
                    .filter(
                        data =>
                            selectedKeys.indexOf(data.key) > -1
                    )
                    .map(data => data.key);
